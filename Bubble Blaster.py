...
sea.pack()
ship_1=sea.create_polygon(0,15,25,25,25,5,fill='red')
ship_2=sea.create_oval(0,0,30,30,outline='red')
...
def move_ship_up(self):
    ...
    corner=place[1]
    if corner > 0 and feeling_brave == True:
...
def move_ship_down(self):
    place=sea.coords(ship_2)
    corner=place[1]
    if corner < 470 and  feeling_brave == True:
        sea.move(ship_1,0,ship_speed/2)
        sea.move(ship_2,0,ship_speed/2)
    window.update()
...
def move_ship_right(self):
    place=sea.coords(ship_2)
    corner=place[0]
    if corner < 770 and feeling_brave == True:
        sea.move(ship_1,ship_speed,0)
        sea.move(ship_2,ship_speed,0)
    window.update()
...
def speed_add_minus(event):
    global ship_speed
    if ship_speed < 20  and feeling_brave == True and event.delta > 0:
        ship_speed+=1
        sea.move(speed_2,9,0)
    elif ship_speed > 0 > event.delta and feeling_brave == True:
        ship_speed-=1
        sea.move(speed_2,-9,0)
